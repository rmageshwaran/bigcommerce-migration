...
"Are there any in-memory caches or shared state that won't work in multi-instance Azure Functions?"
```
...
#### **Multi-Instance Architecture (CRITICAL):**
- [ ] **No in-memory caching** - Azure Functions spin up multiple instances with separate memory spaces
- [ ] **Centralized caching only** - Use services like `IAzureTableInitializationService` for shared caching
- [ ] **Stateless design** - Each function instance must be completely independent
- [ ] **Metrics via logging** - Use structured logging → Application Insights, NOT in-memory metrics
- [ ] **Shared state in external storage** - Azure Table Storage, Cosmos DB, Redis - never local memory
...
```csharp
// ❌ WRONG - In-memory caching breaks with multiple Azure Function instances
private readonly ConcurrentDictionary<string, Data> _cache = new();

// ✅ CORRECT - Use centralized caching or structured logging
private readonly IAzureTableInitializationService _tableService; // Centralized caching
// OR for metrics: Use structured logging → Application Insights (multi-instance aggregation)
_logger.LogDebug("📊 [METRICS] Performance: {ElapsedMs}ms for {Operation}", elapsed, operation);
```
...
- [ ] **Multi-Instance Compliance**: No in-memory shared state, proper Azure Functions scaling design
...
- [ ] **HIGH CONTRAST DIAGRAMS ONLY** (CRITICAL): User requires high contrast colors for text readability - NEVER use low contrast colors that make text difficult to read
